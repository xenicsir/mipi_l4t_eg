...
│
patches/
└── <version>.patch                     # Auto-generated by l4t_copy_sources.sh
                                        # Single unified diff vs original Nvidia BSP
                                        # Applied by l4t_patch_sources.sh
```
...
After copying, the script generates `patches/<version>.patch` — a single unified diff of all Exosens modifications against the original Nvidia BSP. This patch can later be applied to a fresh BSP via `l4t_patch_sources.sh` (faster alternative to re-running `l4t_copy_sources.sh`).
