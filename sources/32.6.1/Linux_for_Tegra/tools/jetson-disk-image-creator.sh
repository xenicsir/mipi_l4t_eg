...
function usage()
{
	...
	echo "			   jetson-xavier-nx-devkit-dione"
	...
	echo "${script_name} -o sd-blob.img -b jetson-agx-orin-devkit -d SD"
	exit 1
}
...
function check_pre_req()
{
	...
	if [ "${board}" == "" ]; then
		...
		jetson-xavier-nx-devkit-dione)
			boardid="3668"
			target="jetson-xavier-nx-devkit-dione"
			storage="sdcard"
			;;
		jetson-agx-orin-devkit)
			boardid="3701"
...
function create_partitions()
{
	...
	for part in "${partitions[@]}"; do
		...
		# part_type will be set by nvptparser.py if a partition type
		# is specified for the partition in the layout file.
		if [ -z "${part_type}" ]; then
