...
then
	...
	# Use Forecr defconfig for Forecr carrier boards, otherwise use tegra_defconfig
	if [[ "$CARRIER_BOARD" == "forecr" ]]; then
		KERNEL_DEFCONFIG=dsboard_ornx_defconfig
	else
		KERNEL_DEFCONFIG=tegra_defconfig
	fi

	pushd $L4T_SRC
	make -C $KERNEL_SOURCES ARCH=arm64 O=$TEGRA_KERNEL_OUT LOCALVERSION=-tegra CROSS_COMPILE=${TOOLCHAIN_PREFIX} $KERNEL_DEFCONFIG
