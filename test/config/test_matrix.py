...
def _find_package(version: str, platform_id: str, vendor: str) -> str | None:
    ...
    if vendor == "forecr":
        ...
        discriminator = "-t186-eg-cams"
    else:
        # generic non-SoM variant — but must NOT match t210/t186/forecr filenames
        candidates = sorted(
            p for p in glob.glob(os.path.join(version_dir, "jetson-l4t-*-eg-cams_*.deb"))
            if "-forecr-" not in os.path.basename(p)
            and "-t210-" not in os.path.basename(p)
            and "-t186-" not in os.path.basename(p)
        )
        return candidates[-1] if candidates else None

    matches = sorted(glob.glob(os.path.join(version_dir, f"jetson-l4t-*{discriminator}_*.deb")))
